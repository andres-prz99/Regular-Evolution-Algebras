...
This repository aims to compute one-dimensional and codimension-one subalgebras of regular evolution algebras over algebraically closed fields, mainly over the complex numbers &#8450;. Recall that a (finite-dimensional) evolution algebra is said to be *regular* if its structure matrix is non-singular. In particular, we implemented two different functions which correspond with the algebraic criteria described in my article:
...
4. **Filtering and Formatting:** It discards the trivial zero solution $(0, \dots, 0)$. For each valid non-zero solution, it outputs a clean dual representation: the raw coordinate values followed by the subalgebra spanned by the corresponding mathematical generator vector $v$ in the natural basis $\\{e_1, \dots, e_n\\}$.
...
* **Case 2: $\text{rank}(M_{p,q}) = 0$**. A family of subalgebras exists, where the generator $v$ is of the form $v = e_p + \lambda e_q$, and $\lambda \neq 0$ is a root of the characteristic cubic equation:
